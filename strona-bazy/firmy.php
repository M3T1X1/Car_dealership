    ...
    <script>
        $(document).ready(function() {
            ...
            $('#confirm-delete-button').on('click', function() {
                ...
                $.ajax({
                    type: "POST",
                    url: "usun_firme.php", // Upewnij się, że masz odpowiedni plik PHP do obsługi usuwania
                    data: { nazwafirmy: nazwafirmy }, // Przekazanie nazwy firmy do usunięcia
                    dataType: "json",
                    success: function(response) {
                        if (response.success) {
                            // Jeśli sukces, zamknij modal, wyświetl komunikat i odśwież stronę
                            alert(response.message); // Komunikat sukcesu
                            $('#delete-company-modal').hide(); // Zamknięcie modala
                    ...
                    error: function(xhr, status, error) {
                        // Obsługa błędu AJAX
                        alert("Wystąpił błąd podczas usuwania firmy. Możliwę, że nip jest w uzyciu.");
                    }
                });
            });
        });
